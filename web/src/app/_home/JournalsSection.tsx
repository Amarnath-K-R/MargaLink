"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { ArrowUpRight } from "lucide-react";
import { between } from "@/lib/easing";
import { loadManifest } from "@/lib/manifest";
import { stagger, motionStyle, countUp } from "./motion.ts";
import { journalCards } from "./demoData.ts";
import { StageLabel } from "./atoms.tsx";

// Static fallback matching the real index as of this build — replaced by
// the live count from loadManifest() once it resolves.
const FALLBACK_JOURNAL_COUNT = 18125;

export default function JournalsSection({
  sectionRef,
  progress,
  next,
  reducedMotion,
}: {
  sectionRef: (el: HTMLElement | null) => void;
  progress: number;
  next: number;
  reducedMotion: boolean;
}) {
  const [journalCount, setJournalCount] = useState(FALLBACK_JOURNAL_COUNT);

  useEffect(() => {
    loadManifest()
      .then((manifest) => setJournalCount(manifest.journal_count))
      .catch(() => {});
  }, []);

  const count = journalCount.toLocaleString("en-US");
  const panelIn = between(progress, 0.65, 0.95);
  return (
    <section id="journals" ref={sectionRef} className="story-section section-shell journal-section">
      <div className="section-stage">
        <div
          className="journals-count"
          aria-hidden="true"
          style={motionStyle(reducedMotion, {
            opacity: between(progress, 0.35, 0.5) * (1 - between(progress, 0.55, 0.75)) * (1 - between(next, 0, 0.5)),
            transform: `translateY(${-between(progress, 0.55, 0.75) * 14}px) scale(${1 - between(progress, 0.55, 0.75) * 0.1})`,
          })}
        >
          <span className="journals-count-number">
            {countUp(journalCount, reducedMotion ? 1 : between(progress, 0.35, 0.5)).toLocaleString("en-US")}
          </span>
          <span className="journals-count-caption">journals, browsable without an upload</span>
        </div>

        <div className="story-grid">
          <div
            className="story-copy"
            style={motionStyle(reducedMotion, {
              opacity: between(progress, 0.25, 1),
              transform: `translateX(${(1 - between(progress, 0.25, 1)) * -36}px)`,
            })}
          >
            <StageLabel number="02" label="Find the right journal" />
            <h2>Start with the landscape, not the upload button.</h2>
            <p>
              Filter {count} journals by field, fees, open access and indexing before your paper goes anywhere. Each
              one links to its own page with the rules that matter for submission.
            </p>
            <Link href="/journals" className="text-link section-link">
              Browse journals <ArrowUpRight size={15} />
            </Link>
          </div>
          <div
            className="journal-panel"
            style={motionStyle(reducedMotion, {
              opacity: panelIn * (1 - next * 0.35),
              transform: `translateX(${(1 - panelIn) * 56}px) scale(${1 - next * 0.06})`,
              filter: `blur(${next * 4}px)`,
            })}
          >
            <div className="panel-topline">
              <span>Journal index · {count} journals</span>
              <span className="panel-tag">Example</span>
            </div>
            <div className="journal-cards">
              {journalCards.map((card, index) => (
                <div
                  className={index === 0 ? "journal-card selected" : "journal-card"}
                  key={card.title}
                  style={motionStyle(reducedMotion, stagger(panelIn, index, journalCards.length, 20))}
                >
                  <div className="card-number">0{index + 1}</div>
                  <div className="card-content">
                    <span className="card-field">{card.field}</span>
                    <strong>{card.title}</strong>
                    <small>{card.fee}</small>
                  </div>
                  <div className="score">
                    <span>{card.score}</span>
                    <small>fit</small>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
