...
export function makePaper(THREE: typeof import("three"), color: number, width: number, height: number): THREE.Group {
...
export function makeInkTrail(THREE: typeof import("three"), color: number, points: THREE.Vector3[]): THREE.Line {
  const geometry = new THREE.BufferGeometry().setFromPoints(points);
  return new THREE.Line(geometry, new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.52 }));
}

export function buildIntroScene(THREE: typeof import("three")) {
  ...
  return { scene, camera, world, heroPaper, sidePaper, lowerPaper, inkGroup, inkDots };
}
